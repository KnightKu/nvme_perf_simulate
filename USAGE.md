...
- `write_page_coalesce=1`（默认，仅 `block_size=0` 且非 codeword 路径）：写按 **cmd_size 分片** 传 DATA，**每 die 凑满一页** 后 **一次 tprog**，host 在 tprog 后完成。
- `write_cache=1`（默认，仅 `block_size=0`）：host 4K 写在 **cmd bus 到达后立刻完成**；后台 **per-die 凑页 flush**（整页 DATA + tprog）。`nand_program_pages` 统计后端 program 次数。
