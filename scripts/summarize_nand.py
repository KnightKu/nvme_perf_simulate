#!/usr/bin/env python3
"""Parse nvme_perf_model NAND-backend logs and write summary CSV/TXT/JSON."""
...
class CaseResult:
    ...
    nand_program_pages: Optional[int] = None
    log_path: str = ""
...
def parse_log(name: str, log_path: Path) -> CaseResult:
    ...
    result.nand_program_pages = _pick_int(text, "nand_program_pages")
...
def write_txt(path: Path, rows: list[CaseResult], out_dir: Path) -> None:
    ...
    lines = [
        "NAND Backend IO Benchmark Summary",
...
def print_table(rows: list[CaseResult]) -> None:
    ...
    print("")
    print("======== NAND Backend IO Benchmark Summary ========")
    print(header)
    print("-" * len(header))
    for row in rows:
        rw = f"{row.read_ratio}/{row.write_ratio}"
...
def write_json(path: Path, rows: list[CaseResult]) -> None:
    payload = {"cases": [asdict(row) for row in rows]}
    path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")


def main() -> int:
    out_dir = Path(sys.argv[1] if len(sys.argv) > 1 else "tests/out_nand")
